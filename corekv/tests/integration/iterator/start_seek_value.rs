...
fn test_start_seek_value<D, S>(mut state: State<D, S>) -> State<D, S>
where
    ...
    S: Snapshot,
{
    ...
    let mut iter = state
        .iter(IterOptions::builder().key_range_start(b"k3").build())
        .expect("create iter");

    assert!(
        iter.seek(b"k1")
            .expect("seek key k3 (since k3 is start of iter range)")
    );
...
tests!(test_start_seek_value: db + snapshot);
