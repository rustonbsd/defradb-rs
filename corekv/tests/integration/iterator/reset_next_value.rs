...
fn test_reset_next_value_partial_iteration<D, S>(mut state: State<D, S>) -> State<D, S>
where
...
fn test_reset_next_value_full_iteration<D, S>(mut state: State<D, S>) -> State<D, S>
where
    D: Db<Snapshot = S, Iter = S::Iter>,
    S: Snapshot,
{
...
tests!(test_reset_next_value_partial_iteration: db + snapshot);
tests!(test_reset_next_value_full_iteration: db + snapshot);
