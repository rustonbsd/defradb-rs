...
fn test_reverse_prefix_set<D, S>(mut state: State<D, S>) -> State<D, S>
where
    ...
    S: Snapshot,
{
    ...
    let mut iter = state
        .iter(IterOptions::builder().prefix(b"k").reverse(true).build())
        .expect("create iter");
...
tests!(test_reverse_prefix_set: db + snapshot);
