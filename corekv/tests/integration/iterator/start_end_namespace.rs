...
fn test_start_end_namespace_excludes_items_outside_of_namespace<D, S>(
    mut state: State<D, S>,
    ...
    S: Snapshot,
{
    ...
    let mut iter = state
        .iter(IterOptions::builder().key_range_start(b"k2").build())
        .expect("create iter");
...
tests!(test_start_end_namespace_excludes_items_outside_of_namespace: db + snapshot | chunk);
