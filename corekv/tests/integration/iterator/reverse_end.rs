...
fn test_reverse_end<D, S>(mut state: State<D, S>) -> State<D, S>
where
...
fn test_reverse_end_single_item_out_of_bounds<D, S>(mut state: State<D, S>) -> State<D, S>
where
    D: Db<Snapshot = S, Iter = S::Iter>,
    S: Snapshot,
{
...
tests!(test_reverse_end: db + snapshot);
tests!(test_reverse_end_single_item_out_of_bounds: db + snapshot);
